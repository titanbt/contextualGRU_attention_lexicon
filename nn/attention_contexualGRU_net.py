...
import theano
from lasagne_nlp.networks.networks import build_Attention_ContexualModel
from nn.contextual.networks import build_Attention_ContexualGRU
import lasagne.nonlinearities as nonlinearities
from lasagne_nlp.utils import utils
from utils.utils import read_model_data

class Attention_ContexualGRU_Net(object):
    def __init__(self, X_train, C_train, embedd_table=None, char_embedd_table = None, label_alphabet=None,
                 ...
                 embedd_lex_dim=15, context_table=None):
    ...
    def buildModel(self):
        ...
        def construct_context_layer():
            layer_context_input = lasagne.layers.InputLayer(shape=(None, max_length), input_var=self.input_var, name='input')
            layer_context_embedding = lasagne.layers.EmbeddingLayer(layer_context_input, input_size=alphabet_size,
                                                            output_size=embedd_context_dim,
                                                            W=self.context_table, name='embedding')
            return layer_context_embedding
        ...
        _, embedd_context_dim = self.context_table.shape
        ...
        layer_context = construct_context_layer()

        layer_mask = lasagne.layers.InputLayer(shape=(None, max_length), input_var=self.mask_var, name='mask')

        # construct model
        model = build_Attention_ContexualGRU(layer_char, layer_word, layer_lex, layer_context, self.num_units, mask=layer_mask,
        ...
        self.train_fn = theano.function([self.input_var, self.target_var, self.mask_var, self.char_input_var, self.lex_var],
                                   outputs=self.loss_train,
                                   updates=updates, allow_input_downcast=True)
        # Compile a second function evaluating the loss and accuracy of network
        self.eval_fn = theano.function([self.input_var, self.target_var, self.mask_var, self.char_input_var, self.lex_var],
                                  outputs=correct_predictions, allow_input_downcast=True)

        self.test_fn = theano.function([self.input_var, self.target_var, self.mask_var, self.char_input_var, self.lex_var],
