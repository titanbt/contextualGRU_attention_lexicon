...
import theano
from nn.contextual.networks import build_ContextualGRU
import lasagne.nonlinearities as nonlinearities
from lasagne_nlp.utils import utils
from utils.utils import read_model_data

class ContexualGRU_Net(object):
    ...
    def buildModel(self):
        ...
        # construct model
        model = build_ContextualGRU(layer_word, layer_context, self.num_units, mask=layer_mask,
                                    grad_clipping=self.grad_clipping, dropout=self.dropout)
        ...
        self.eval_fn = theano.function([self.input_var, self.target_var, self.mask_var],
                                  outputs=correct_predictions, allow_input_downcast=True)

        self.test_fn = theano.function([self.input_var, self.target_var, self.mask_var],
